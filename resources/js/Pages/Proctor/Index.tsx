...
import { Calendar, Clock, BookOpen, Monitor, Users } from 'lucide-react';
...
export default function Index({ exams }: Props) {
    const getStatus = (startTime: string, endTime: string) => {
        ...
        if (now < start) return { label: 'Upcoming', color: 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-400' };
        if (now > end) return { label: 'Finished', color: 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-400' };
                                    ...
                                    >
                                        <div className="border-b border-gray-100 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-700/50">
                                            <div className="flex items-center justify-between">
                                                ...
                                                <span className={`inline-flex items-center rounded-md px-2 py-1 text-xs font-medium ${status.color}`}>
                                                    {status.label}
                                                </span>
                                            </div>
                                        ...
                                        <div className="flex flex-1 flex-col p-4">
                                            ...
                                            <div className="mt-auto flex gap-2">
                                                <button
                                                    className="flex flex-1 items-center justify-center gap-2 rounded-lg bg-primary-600 px-4 py-2 text-sm font-medium text-white hover:bg-primary-700 focus:outline-none focus:ring-4 focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800"
                                                >
                                                    <Monitor className="h-4 w-4" />
                                                    Monitor
                                                </button>
                                                <button
                                                    className="flex items-center justify-center rounded-lg border border-gray-200 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-4 focus:ring-gray-100 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700 dark:focus:ring-gray-700"
                                                >
